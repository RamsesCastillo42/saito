...
function Facebook(app) {
  ...
  this.remoted_loaded    = 0;
  this.facebook.deleted  = [];  // txids of deleted posts which are
				// loaded from the server
  this.deleted           = []; // txids of deleted posts which are
				// loaded from the server
...
Facebook.prototype.updateDeleted = function updateDeleted(nd) {
  this.facebook.deleted = nd;
  this.saveFacebook(this.app);
}
...
Facebook.prototype.initializeHTML = function initializeHTML(app) {
  ...
  if (app.options.facebook != null) {
    ...
    this.facebook.deleted  = app.options.facebook.deleted;
  ...
  <div class="comment" id="comment_${msg.id}">
    <table>
      <tr>
        ...
        <div class="comment_name" id="comment_name_${msg.id}">${this.formatAuthor(msg.from, app, msg)}</div> - <div class="comment_text">${linkifyHtml(markdown.toHTML(msg.data))}</div>
        </td>
      </tr>
    </table> 
  </div> \
...
Facebook.prototype.formatPost = function formatPost(msg, app) {
  var html = `
   ...
   if (msg.markdown == 0) {
      ...
      html += `<div class="post_content">${linkifyHtml(markdown.toHTML(msg.data))}</div>`;
   }
   html += `
...
';
  ...
  $('.post_controls_delete').on('click', function() {
    ...
    // add delete for remote-load items
    facebook_self.facebook.deleted.push(msgid);
    facebook_self.saveFacebook(facebook_self.app);
...
Facebook.prototype.handlePeerRequest = function handlePeerRequest(app, message, peer, mycallback) {
  ...
  if (message.request === "facebook load") {
    ...
    id = newtx.transaction.id;

    //
    // if we are loading a tx that we have deleted, keep the
    // record around
    //
    if (this.facebook.deleted.includes(id.toString())) {
    ...
    //
    // when all are loaded, purge anything we deleted that isn't 
    // being fed to us still
    //
    this.remote_loaded++;
    if (this.remote_loaded == 5) {
      this.facebook.deleted = this.deleted;
      this.saveFacebook(this.app);
    }
