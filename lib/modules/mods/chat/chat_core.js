...
class ChatCore extends ModTemplate {
  ...
  _receiveMessage(app, tx) {
    ...
    if (!this.app.BROWSER) {
      var newtx = this._createRelayedMessageTX(tx, txmsg);
      if (!newtx) { return; }
      this._sendMessageToPeers(newtx);
      this._saveMessageToDB(newtx);
    } else {
  ...
  _createRelayedMessageTX(tx, txmsg) {
    // room_publickey??
    let { room_id, room_publickey, publickey, message, sig } = txmsg;

    var newtx = this.app.wallet.createUnsignedTransaction(room_publickey, 0.0, 0.0);
  ...
  _sendCreateRoomRequest(addresses) {
    ...
    this.app.network.sendRequest("chat request create room", data);
  }
  ...
  _sendCreateRoomResponse(app, tx, peer) {
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      app.network.sendRequest("chat response create room", data);
    }
  }
