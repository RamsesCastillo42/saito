...
class ChatCore extends ModTemplate {
  ...
  _getChatURL() {
    return `${this.server.peer.protocol}://${this.server.peer.host}:${this.server.peer.port}`
  }
  ...
  async _sendMessage(chat_room_id, msg) {
    let fee = await this._returnModServerStatus() ? 0.0 : 2.0;
  ...
  _saveChat() {
    ...
    this.app.options.chat = Object.assign({}, {server, rooms: this._getRoomsWithoutMessages()});
