...
Happy Chatting!`
  ...
  async _addMessageToRoom(tx, room_idx) {
    ...
    lastMessage.author = await this._getIdentifier(publickey);
    this.chat.rooms[room_idx].messages[this.chat.rooms[room_idx].messages.length-1].author = lastMessage.author;

    // Need some conditions on Notification
    let current_room = $('.chat_messages-list')[0];
    if (current_room.id != room_id || $('.mail_chat_popup').css("bottom") == "40px") {
      this.app.browser.notification(`${lastMessage.author} in ${this.chat.rooms[room_idx].name}`, lastMessage.message);
