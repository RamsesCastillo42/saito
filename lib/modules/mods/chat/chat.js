...
Happy Chatting!`
  ...
  _handleCreateRoomResponse(app, tx) {
    ...
    if (new_room) {
      ...
      let notify = this.app.browser.notification(`New Room created: ${new_room.name}`, "");
      notify.onclick = (event) => { this._onclickChatNotification(new_room.room_id); }
    }
  }
  ...
  _sendNewMessageNotification(room_idx, lastMessage) {
    ...
    if (current_room.id != room.room_id || isPopupHidden || !document.hasFocus()) {
      ...
      let notify = this.app.browser.notification(notification_title, lastMessage.message);
      notify.onclick = (event) => { this._onclickChatNotification(room.room_id); }
  ...
  _onclickChatNotification(room_id) {
    ...
    if (isPopupHidden) { this._toggleMailchat(); }
    this._scrollToBottom();
