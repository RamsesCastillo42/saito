...
class Chat extends ChatCore {
  ...
  async _addMessageToRoom(tx, room_idx) {
    ...
    // let last_message = this.chat.rooms[room_idx].messages[this.chat.rooms[room_idx].messages.length-1];
    this.chat.rooms[room_idx].messages[this.chat.rooms[room_idx].messages.length-1] = await this._getIdentifier({sig, author: publickey, message});
