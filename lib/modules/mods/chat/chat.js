...
class Chat extends ChatCore {
  ...
  initialize() {
    ...
    this.server_processing = setInterval(() => {
      ...
      if (this.server) clearInterval(this.server_processing);
    }, 500);
  ...
  _welcomeMessage() {
    return {
      ...
      author: 'BearGuy',
      message:
      `Welcome to Saito Chat! Please note that this is an unencrypted platform (for the time being) and we can't guarantee that your message will be delivered unless you provide a fee. \n\n
      Here are some commands to get you started \n
      "add 21959827AE41923837D498234CE4719238123" -- start private chat with someone's public key \n
      "add david@saito" -- start private chat with someone's identifier \n
      "add david@saito richard@saito 'Saito Squad'" -- start group chat and name it (make sure to use single quotes) \n\n
      Happy Chatting!`
    }
  }
  ...
  async _addSendEvent(message, room_id) {
    ...
    var newtx = await this._sendMessage(room_id, message);
