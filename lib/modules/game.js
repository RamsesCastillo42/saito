...
Game.prototype.handleOnConfirmation = function handleOnConfirmation(blk, tx, conf, app) {
  ...
  if (conf == 0) {
    ...
    if (txmsg.request == "game") { if (txmsg.step.game != undefined) { if (txmsg.step.game <= this.game.step.game) { return; } }
...
Game.prototype.rollDice = function rollDice(sides=6, mycallback=null) {
  ...
  let a = parseInt(this.dice.slice(0,12), 16)%sides;
...
Game.prototype.handleDeck = function handleDeck(msg, mycallback=null) {
  ...
  if (this.game.player == 1) {
    ...
    //
    // player1 continues the game after every shuffle
    //
    msg.extra.target = 1;
    this.handleGame(msg);
...
Game.prototype.initializeDeck = function initializeDeck(cards=null) {
  ...
  this.updateStatus("initializing our deck of cards...");
...
Game.prototype.handleDeal = function handleDeal(msg) {
  ...
  if (this.game.player != receiver) {
    ...
    //
    // someone has sent me their keys, 2 player for now
    //
    // TODO - generalize to N player exchanges
    //
    if (keys.length > 0) {
...
console.log("SEND MESSAGE 2: " + JSON.stringify(newtx.transaction));
