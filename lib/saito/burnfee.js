...
BurnFee.prototype.burnFeeCurve = function burnFeeCurve(bf, elapsed_time) {
  ...
  // Smallest difference we can possibly have is 1
  ...
  return bf.start/(Math.abs(elapsed_time)/1000);
}
...
BurnFee.prototype.burnFeeAdjustment = function burnFeeAdjustment(prevblk=null, blk=null) {
  ...
  return prevblk.block.bf.start * Math.sqrt((this.heartbeat * 1000)/(Math.abs(blk.block.ts - prevblk.block.ts) + 1));
}
/**
