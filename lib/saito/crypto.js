...
//
// https://github.com/thomaschampagne/crypto-xor/blob/master/crypto-xor.js
//
// under MIT license
//
...
Crypto.prototype.encodeXOR = function encodeXOR(plaintext, key) {
    ...
    for (var i = 0; i < plaintext.length; i++) {
        ...
        var dec = (plaintext[i]).charCodeAt(0) ^ (key[keyPointer]).charCodeAt(0);
        dec = parseInt(dec);
...
Crypto.prototype.decodeXOR = function decodeXOR(string, key) {
    ...
    for (i = 0; i < string.length; i = i + 2) {
        cypherArray.push(string[i] + string[i + 1]);
