...
/**
 * fetchIndentifier wrapped with Promise
 *
 * @params {string} publickey
 * @params {callback}
 *
**/
DNS.prototype.fetchIdentifierPromise = function fetchIdentifierPromise(publickey) {
  return new Promise((resolve, reject) => {
    this.fetchIdentifier(publickey, (answer) => {
      var author = this.isRecordValid(answer) ?  JSON.parse(answer).identifier : publickey
      resolve (author);
    });
  });
}
 ...
 *
**/
DNS.prototype.fetchMultipleIdentifiers = function fetchMultipleIdentifiers(publickeys, mycallback) {
  ...
  for (let s = 0; s < this.dns.domains.length; s++) {
    for (let t = 0; t < this.app.network.peers.length; t++) {
      if (this.dns.domains[s].publickey == this.app.network.peers[t].peer.publickey) {
        ...
        // find out initial state of peer and blockchain
        var userMessage = {};
            userMessage.request          = "dns multiple";
            userMessage.data             = {};
            userMessage.data.request     = "dns";
            userMessage.data.publickeys  = publickeys;
...
/**
 * fetchPublicKey wrapped with Promise
 *
 * @params {string} identifier
 * @params {callback}
 *
**/
DNS.prototype.fetchPublicKeyPromise = function fetchPublicKeyPromise(identifier) {
  return new Promise((resolve, reject) => {
    this.fetchPublicKey(identifier, (answer) => {
      var author = this.isRecordValid(answer) ? JSON.parse(answer).publickey : identifier
      resolve (author);
    });
  });
}
...
DNS.prototype.isRecordValid = function isRecordValid(answer) {
  ...
  if (obj.payload) {
    return obj.payload.every(key => {
      let msgtoverify = key.identifier + key.publickey + key.block_id + key.block_hash;
      return this.app.crypto.verifyMessage(msgtoverify, key.signature, key.signer);
    })
  }
