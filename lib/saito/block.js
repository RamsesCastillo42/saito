...
function Block(app, blkjson = "", confirmations = -1) {
  ...
  this.created_on_empty_mempool = 0; // this variable is used to detect conflicts 
				     // that might happen if I create a block on 
				     // an empty mempool, but then download one
				     // before it is finished and thus my block
				     // is never longest-chained
				     //
				     // setting this variable and then checking
				     // to see if there are more blocks in the 
				     // mempool helps me avoid having my txs
				     // NEVER recovered because my block is never
				     // added to the longest chain
 ...
 */
Block.prototype.hasKeysInTransactions = function hasKeysInTransactions(keylist) {
  let tempblk = JSON.parse(JSON.stringify(this.block));
