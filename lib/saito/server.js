...
Server.prototype.initialize = function initialize() {
  ...
  app.post('/blocks', async (req, res) => {
    ...
    blocks = block_hashes.map(async bhash => {
      if (bhash) {
        try {
          if (publickeys != [] || publickeys == undefined) {
            let blk = await this.app.blockchain.returnBlockByHash(bhash, 1);
            return blk.hasKeysInTransactions(publickeys)
          } else {
  ...
  // blocks //
  ////////////
  app.get('/blocks/:bhash', async (req, res) => {
  ...
  app.post('/lite-blocks/:pkey', async (req, res) => {
    ...
    try {
      blocks = block_hashes.map(async bhash => {
        ...
        return blk.hasKeysInTransactions(keylist)
      });
