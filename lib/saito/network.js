...
console.log("monitor peers 1");
    for (let i = this.peers.length-1; i >= 0; i--) {
console.log("monitor peers 2: " + i + " -- " + this.peers.length);
...
Network.prototype.isPrivateNetwork = function isPrivateNetwork() {
  ...
  for (let i = 0; i < this.peers.length; i++) {
console.log("is private network? " + i + " -- " + this.peers.length);
...
Network.prototype.sendTransactionToPeers = function signTXAndSendToPeers(tx, outbound_message, callback=null) {
  this.peers.forEach((peer) => {
    if (!peer.inTransactionPath(tx)) {
      tx = peer.addPathToTransaction(tx);
      if (callback) {
        peer.sendRequestWithCallback(outbound_message, JSON.stringify(tx.transaction), callback);
      } else {
        peer.sendRequest(outbound_message, JSON.stringify(tx.transaction));
      }
    }
  });
}
