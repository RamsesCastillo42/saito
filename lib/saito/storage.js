...
Storage.prototype.validateTransactionInput = function validateTransactionInput(slip, bid) {
  ...
  if (slip.amt > 0) {
console.log("HERE 2: " + slip.returnIndex());
console.log("HERE 2.5: " + shashmap.validate_slip(slip.returnIndex(), bid));
console.log("HERE 2.7: " + this.returnShashmapValue(slip));
    if (shashmap.validate_slip(slip.returnIndex(), bid) == 0) {
console.log("HERE 3: " + slip.bid + " -- " + bid + " -- " + this.app.blockchain.genesis_period);
...
Storage.prototype.onChainReorganization = async function onChainReorganization(block_id, block_hash, lc) {
  ...
  if (this.app.BROWSER == 0) {
    ...
    // update shashmap
    //
    // longest-chain outputs set to -1 (unspent), non-longest-chain outputs set to -2 (unspendable)
    //
    for (let b = 0; b < blk.transactions.length; b++) {
      for (let bb = 0; bb < blk.transactions[b].transaction.to.length; bb++) {
        if (blk.transactions[b].transaction.to[bb].amt > 0) {
          ...
          var slip_map_index = blk.transactions[b].transaction.to[bb].returnIndex();
	  if (lc == 1) {
console.log("UPDATING SLIP " + slip_map_index + " with value -1");
            shashmap.insert_slip(slip_map_index, -1);
	  } else {
console.log("UPDATING SLIP " + slip_map_index + " with value -2");
            shashmap.insert_slip(slip_map_index, -2);
	  }
        }
      }
    }
