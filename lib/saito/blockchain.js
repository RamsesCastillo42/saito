...
console.log(" .... run callbks: " + new Date().getTime());
  ...
  if (this.blocks.length > this.callback_limit) {
    ...
    if (blk2clear >= 0) {
console.log(this.blocks.length + "---" + this.callback_limit);
console.log("CLEARING BLOCK AT: " + blk2clear);
console.log("ID GETTING PURGED: " + this.blocks[blk2clear].block.id);
      this.blocks[blk2clear].transactions = [];
      this.blocks[blk2clear].block.txsjson = [];
      // sanity check for blocks added earlier
      if (pos < blk2clear) {
console.log("POS is less than blk2clear: " + pos);
...
Blockchain.prototype.returnBlockByHash = async function returnBlockByHash(hash, txjson=0) {
  ...
  //
console.log("is hash indexed: " + hash + " -- " + txjson);
  if (this.isHashIndexed(hash)) {
//console.log("(yes)");
    for (let v = this.blocks.length-1; v >= 0; v-- ) {
      if (this.blocks[v].hash == hash) {
        if (txjson == 0) {
console.log("returning locally stored version: " + txjson);
//console.log("this: " + JSON.stringify(this.blocks[v].transactions));
