...
console.log(" .... AAA 1");
console.log(JSON.stringify(game_self.game));
...
console.log("ACCEPT TXMSG request 1: ");

console.log("GAME OBJ: " + JSON.stringify(game_self.game));

	  if (game_self.game.step.game == 0) {

console.log("ACCEPT TXMSG request 2: ");

            if (tx.transaction.from[0].add == app.wallet.returnPublicKey()) {
              game_self.game.accept = 1;
console.log("I ACCEPTED THIS GAME!");
              if (game_self.app.network.isConnected() == 1) {
                game_self.saveGame(game_id);
              }
            }

console.log("ACCEPT TXMSG request 3: ");

            for (let i = 0; i < tx.transaction.to.length; i++) { game_self.addOpponent(tx.transaction.to[i].add); }
            game_self.game.module = txmsg.module;
            game_self.saveGame(game_id);

console.log("ACCEPT TXMSG request 4: ");
...
console.log("ACCEPT TXMSG request 5: " + originator);
...
console.log("now we have: " + JSON.stringify(game_self.game));
...
console.log("NOW: " + JSON.stringify(game_self.game.accepted));

console.log("NOW WE ARE SORTING TO DETERMINE PLAY ORDER");
...
console.log("pst initialize game");
