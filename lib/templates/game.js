...
console.log("POST IGF 3");
        if (txmsg.step.game != undefined) {
console.log("POST IGF 4");
          if (txmsg.step.game <= game_self.game.step.game) {
console.log("GAME SAVE 1");
            return;
          }
        } else {
console.log("POST IGF 5");
	  txmsg.step.game = 0;
console.log("POST IGF 6");
          if (game_self.game.step.game > 0) {
console.log("GAME SAVE 2");
            return;
          }
	}
console.log("POST IGF 7");
        if (txmsg.extra != undefined) {
console.log("POST IGF 8");
          if (txmsg.extra.target != undefined) {
            game_self.game.target = txmsg.extra.target;
          }
        } else { txmsg.extra = {}; }
console.log("POST IGF 9");
	if (txmsg.turn == undefined) { txmsg.turn = []; }
console.log("POST IGF 10");
...
console.log("POST IGF 3");
console.log("1: " + JSON.stringify(game_self.game.queue));
        ///////////
        // QUEUE //
        ///////////
        if (game_self.game.queue != undefined) {

console.log("2: " + JSON.stringify(game_self.game.queue));
...
console.log("3: " + JSON.stringify(game_self.game.queue));

  if (game_self.game.queue.length > 0) {
console.log("4: " + JSON.stringify(game_self.game.queue));
    while (game_self.game.queue.length > 0 && cont == 1) {

console.log("in parent loop: " + JSON.stringify(game_self.game.queue));
...
console.log("CONT 3: " + JSON.stringify(txmsg));
