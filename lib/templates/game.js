...
Game.prototype.initializeHTML = function initializeHTML(app) {
  ...
  // insert HUD
  //
  let hud_size = window.matchMedia("(orientation: landscape)").matches ? "wide" : "tall";
  let hud_caret = window.matchMedia("(orientation: landscape)").matches && this.app.browser.isMobileBrowser(navigator.userAgent) ? "right" : "down";
...
console.log("ASSIGNING NEW TXMSG: " + JSON.stringify(txmsg));
...
console.log("SAVED AS: " + JSON.stringify(game_self.game.last_txmsg));
console.log("GAME_ID: " + game_self.game.id);
...
this.updateLog("deck initial card encrypt...");
      ...
      //
      if (cont == 1) {
console.log("CONT 1: " + JSON.stringify(txmsg) + " -- " + JSON.stringify(game_self.game.last_txmsg));
...
console.log("CONT 2: " + JSON.stringify(txmsg) + " -- " + JSON.stringify(game_self.game.last_txmsg));
