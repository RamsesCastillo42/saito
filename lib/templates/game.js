...
Game.prototype.onConfirmation = async function onConfirmation(blk, tx, conf, app) {
  ...
  if (conf == 0) {
	...
	if (game_self.low_balance_alert_sent == 0 && Big(app.wallet.returnBalance()).lt(80)) {
	  ...
	  if (txmsg.reason != undefined) {
	    game_self.updateStatus(txmsg.reason);
	    game_self.updateLog(txmsg.reason);
	  }
...
console.log("HALFWAY DONE IN GAME>JS");

  newtx.transaction.msg.module  = this.game.module;
console.log("DONE IN GAME>JS 1");
  newtx.transaction.msg.request = "gameover";
console.log("DONE IN GAME>JS 2");
  newtx.transaction.msg.game_id = this.game.id;
console.log("DONE IN GAME>JS 3");
  newtx.transaction.msg.reason  = reason;
console.log("DONE IN GAME>JS 4");
  newtx = this.app.wallet.signTransaction(newtx);
console.log("DONE IN GAME>JS 5");
  this.app.network.propagateTransaction(newtx);
console.log("DONE IN GAME>JS 6");
