...
console.log("STILL HERE: " + JSON.stringify(game_self.game));
...
console.log("EXECUTING EMAIL!");
        if (gmv[1] == "ready") {
console.log("EXECUTING EMAIL ready!");
...
console.log("UNSHIFT: " + game_self.old_crypt[i] + " --- " + game_self.old_keys[i]);
...
console.log("RE-ADD: " + game_self.old_cards[b]);
...
Game.prototype.saveGame = function saveGame(game_id = null) {
  ...
  if (this.app.options.games == undefined) { 
console.log("games is undefined!");
  ...
  if (game_id != null) {
    for (let i = 0; i < this.app.options.games.length; i++) {
      if (this.app.options.games[i].id == game_id) {
console.log("... saving game: " + game_id);
