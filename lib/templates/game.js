...
Game.prototype.attachEvents = function attachEvents(app) {
  ...
  $('#game_status').on('click', () => {
    if ((this.app.browser.isMobileBrowser(navigator.userAgent) && screen.orientation.angle == 0) || (window.innerHeight > 700)) {
  ...
  window.addEventListener("orientationchange", function(e) {
    console.log("ORIENTATION OF THE SCREEN: ", screen.orientation.angle);
...
console.log("WHAT IS THE SIG: " + txmsg.sig);
	  ...
	  if (txmsg.ts > 0) {
		...
		return;
	        ...
	        game_self.game.invitesig = invite_sig;
	  ...
	  //
	  if (old_game_id != "") {
console.log("LOADING OLD GAME: " + old_game_id);
...
console.log("LOADING OLD GAME 2: " + game_id);
          game_self.loadGame(game_id);
...
console.log("LOADING OLD GAME 3: " + game_id);
          game_self.loadGame(game_id);
	...
	if (txmsg.game_id != undefined) {
	  if (txmsg.game_id != "") {
	    if (game_self.game.id != txmsg.game_id) {
	      ...
	      console.log("LOADED GID: " + game_self.game.id);
	    }
	  }
	}

//        if (game_self.game.id != undefined) {
//          if ( game_self.initializeGameFeeder(game_self.game.id) == 1 ) {} else {
...
console.log("resetting games to undefined...");
  ...
  if (game_id == null) {
    ...
    if (this.app.options.games == undefined) {
console.log("game id is null AND setting our options is undefined...");
      game_id = null;
    } else {
  ...
  //
console.log("Kicking into New Game with GID: " + game_id);
...
Game.prototype.newGame = function newGame(game_id = null) {
  ...
  if (game_id == null) { game_id = Math.random().toString(); 
    console.log("CREATED NEW GAME WITH NULL STRING!");
  }
...
Game.prototype.initializeGameFeeder = function initializeGameFeeder(game_id) {
  ...
  if (this.game.initialize_game_run == 1) { return 0; } else { this.game.initialize_game_run = 1; }
...
console.log("INITIALIZE GAME!");
  ...
  //
  //
  console.log("INIT GAME FEEDER: " + JSON.stringify(this.game.queue));
