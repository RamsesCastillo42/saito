...
Game.prototype.attachEvents = function attachEvents(app) {
  ...
  $('#game_help').on('click', function () {
	...
	user_message += '<div><span id="restoreGameState" title="Restore Game State" class="fa fa-file-alt hud-button">R </span><span class="interfacekeytip keytip">Restore Frozen Game (beta)</span></div>';
    }
...
console.log(response);
        alert("keystate restored....");
        game_self.restoreKeyState(response);
        $.get(`/arcade/restore/${game_self.game.id}`, (response, error) => {
console.log(response);
	  let newgame = JSON.parse(response);
	  if (newgame.player != game_self.player) {
	    ...
	    //
            window.location = '/' + game_self.module;
	  } else {
	    alert("my last saved move is my last move...");
	  }
        });
      });
    });
...
console.log("\n\nCOMPARING DECK LENGTHS: " + opponent_deck_length + " -- " + my_deck_length + " <--- if these differ we may have a bug!");
	...
	// thorny bug
	//
	// if the first player is waiting for the second and reloads....
        //
	// irregular hangs if it does not
	//
	//game_self.game.last_txmsg = "";
        //game_self.saveGame(game_self.game.id);
