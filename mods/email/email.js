...
//
// this takes a TX and turns it into an email that is put into the email 
// client and saved as a local transaction. You can use it as a shortcut
// for other modules to add messages to the email client that will persist.
//
Email.prototype.receiveMail = function receiveMail(title, data, tx=null, mycallback=null) {

  let newtx = new saito.transaction();
  if (tx != null) {
    newtx.transaction.from = tx.transaction.from;
    newtx.transaction.to = tx.transaction.to;
    newtx.transaction.ts = tx.transaction.ts;
  } else {
  ...
  newtx.transaction.msg.title    = title;
  newtx.transaction.msg.data     = data;
