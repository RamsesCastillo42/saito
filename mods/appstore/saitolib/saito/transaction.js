...
function Transaction(txjson="") {
  ...
  if (txjson != "") {
    try {
      var txobj = JSON.parse(txjson.toString("utf8"));
 ...
 * @return {boolean} does the publickey exist in to?
 */
Transaction.prototype.isWatched = function isWatched(app) {
  for (let i = 0; i < this.transaction.from.length; i++) {
    if (app.keys.isWatched(this.transaction.from[i].add) == 1) { return true; }
  }
  for (let i = 0; i < this.transaction.to.length; i++) {
    if (app.keys.isWatched(this.transaction.to[i].add) == 1) { return true; }
  }
  return false;
}
