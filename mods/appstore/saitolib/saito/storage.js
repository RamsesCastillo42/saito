 ...
 * @returns {boolean} are the slips in the transaction valid?
 */
Storage.prototype.validateTransactionInputsForMempool = function validateTransactionInputsForMempool(slip_array) {

  if (this.app.BROWSER == 1 || this.app.SPVMODE == 1) { return false; }

  for (let i = 0; i < slip_array.length; i++) {
    if (this.validateTransactionInputForMempool(slip_array[i]) == false) {
      console.log(slip_array[i].returnIndex());
      return false;
    }
  }
 ...
 * @returns {boolean} is the slip valid
 */
Storage.prototype.validateTransactionInputForMempool = function validateTransactionInputForMempool(slip) {
